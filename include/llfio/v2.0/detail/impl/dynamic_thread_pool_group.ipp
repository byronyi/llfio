...
namespace detail
{
  ...
  struct global_dynamic_thread_pool_impl
  {
    ...
    inline void _submit_work_item(_lock_guard &g, bool item_in_timer_list, bool submit_into_highest_priority, dynamic_thread_pool_group::work_item *workitem, bool defer_pool_wake);
...
namespace detail
{
  ...
  inline void global_dynamic_thread_pool_impl::_submit_work_item(_lock_guard &g, bool item_in_timer_list, bool submit_into_highest_priority, dynamic_thread_pool_group::work_item *workitem,
                                                                 bool defer_pool_wake)
  {
    ...
    if(workitem->_nextwork != -1)
    {
      ...
      if(workitem->_nextwork == 0 || workitem->_has_timer_set())
      {
        ...
        {
          _remove_from_list(parent->_work_items_active, workitem);
          _append_to_list(parent->_work_items_timer, workitem);
        }
#if LLFIO_DYNAMIC_THREAD_POOL_GROUP_USING_GCD
      ...
      else
      {
        if(item_in_timer_list)
        {
          _remove_from_list(parent->_work_items_timer, workitem);
          _append_to_list(parent->_work_items_active, workitem);
        }
#if LLFIO_DYNAMIC_THREAD_POOL_GROUP_USING_GCD
